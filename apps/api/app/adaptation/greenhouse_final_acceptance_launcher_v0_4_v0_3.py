...
GREENHOUSE_FINAL_ACCEPTANCE_LAUNCHER_RULE_VERSION = (
    "qlora_v0.4_greenhouse_final_acceptance_launcher_v0.3"
)
...
# ============================================================
# Metadata-only protected case-manifest remediation v0.3
# ============================================================

def _metadata_manifest_contains_json_list(
    value: Any,
) -> bool:
    ...
    if isinstance(
        ...
        return any(
            _metadata_manifest_contains_json_list(
                child
            )
...
def _metadata_manifest_collect_keys(
    value: Any,
) -> set[
    str
]:
    ...
    if isinstance(
        ...
        for key, child in value.items():
            ...
            output.update(
                _metadata_manifest_collect_keys(
                    child
                )
            )
...
def validate_metadata_only_case_manifest(
    payload: Any,
    *,
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "Protected Greenhouse case manifest must be an object."
        )


    if _metadata_manifest_contains_json_list(
        payload
    ):

        raise RuntimeError(
            (
                "Protected Greenhouse case manifest v0.3 "
                "must be metadata-only and contain no JSON lists."
            )
        )
    ...
    if unexpected_case_keys:
        ...
        raise RuntimeError(
            (
                "Protected Greenhouse metadata-only case manifest "
                "contains case-level fields: "
                f"{sorted(unexpected_case_keys)}"
            )
        )
    ...
    exact_identity_values = {
        ...
        "dataset_column_count":
            len(
                benchmark_module.COLUMNS
            ),

        "dataset_row_count":
            len(
                benchmark_module.ROWS
            ),
    }


    for key, expected_value in (
        exact_identity_values.items()
    ):
    ...
    for key, expected_value in (
        ...
        if payload.get(
            ...
            raise RuntimeError(
                (
                    "Protected Greenhouse case-manifest "
                    f"boolean contract mismatch: {key}."
                )
            )
    ...
    if (
        ...
        raise RuntimeError(
            "Protected Greenhouse case-manifest domain changed."
        )
    ...
    if (
        ...
        or
        not evaluation_role.strip()
    ):

        raise RuntimeError(
            "Protected Greenhouse evaluation role is missing."
        )


    technical_split = payload.get(
        "technical_split"
    )
    ...
    if (
        ...
        or
        not technical_split.strip()
    ):

        raise RuntimeError(
            "Protected Greenhouse technical split is missing."
        )


    independence_scan = payload.get(
        "independence_scan"
    )
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "Protected Greenhouse independence scan metadata missing."
        )


    if independence_scan.get(
        "match_count"
    ) != 0:

        raise RuntimeError(
            "Protected Greenhouse independence match count is not zero."
        )
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "Protected Greenhouse methodology metadata missing."
        )


    preregistered_gates = payload.get(
        "preregistered_gates"
    )
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "Protected Greenhouse preregistered-gates metadata missing."
        )
    ...
    for benchmark_case in benchmark_cases:
        ...
        if case_id in seen_case_ids:
            ...
            raise RuntimeError(
                "Frozen benchmark case IDs are not unique."
            )
        ...
        if (
            ...
            raise RuntimeError(
                (
                    "Frozen benchmark case dataset identity "
                    f"changed: {case_id!r}."
                )
            )
        ...
        if (
            ...
            raise RuntimeError(
                (
                    "Frozen benchmark left column is absent "
                    f"from dataset columns: {case_id!r}."
                )
            )
        ...
        if (
            ...
            raise RuntimeError(
                (
                    "Frozen benchmark right column is absent "
                    f"from dataset columns: {case_id!r}."
                )
            )
    ...
    if len(
        ...
        raise RuntimeError(
            "Frozen benchmark unique case count changed."
        )
...
def build_benchmark_case_identity_entries(
    *,
    benchmark_module: Any,
) -> list[
    dict[
    ...
    for benchmark_case in benchmark_cases:
        ...
        if (
            ...
            raise RuntimeError(
                (
                    "Frozen benchmark case uses different "
                    f"dataset identities: {case_id!r}."
                )
            )
    ...
    if len(
        ...
        raise RuntimeError(
            "Benchmark case-identity count changed."
        )
...
def execute_greenhouse_final_acceptance(
    *,
    ...
    benchmark_case_identity_entries = (
