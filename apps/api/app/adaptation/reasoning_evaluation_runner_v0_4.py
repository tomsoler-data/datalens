...
# ============================================================
# QLORA v0.4 HOTEL DIAGNOSTIC PREREQUISITES
# ============================================================

HISTORICAL_SCORING_RUNNER_PATH = (
    ROOT
    / "app"
    / "adaptation"
...
S3_REPORT_PATH = (
    ...
    / (
        "datalens_semantic_qlora_v0.4_"
        "s3_post_training_gate_v0.1_report.json"
    )
)
...
S3_RECEIPT_PATH = (
    ...
    / (
        "datalens_semantic_qlora_v0.4_"
        "s3_post_training_gate_v0.1_receipt.json"
    )
)
...
def validate_v0_4_diagnostic_prerequisites(
) -> None:
    if (
        sha256_file(
            HISTORICAL_SCORING_RUNNER_PATH
        ...
        raise RuntimeError(
            "Historical Hotel scoring runner changed."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Official v0.4 training report changed."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Official v0.4 training receipt changed."
        )
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "Official adapter evidence is malformed."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Official v0.4 adapter bundle SHA changed."
        )
    ...
    for (
        ...
        if not path.is_file():
            raise RuntimeError(
                (
                    "Official adapter file is missing: "
                    f"{filename}"
                )
            )
    ...
    if (
        ...
        raise RuntimeError(
            "Hotel diagnostic blocked by S3 gate."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Hotel diagnostic blocked by S3 failures."
        )
    ...
    if (
        ...
        raise RuntimeError(
            (
                "Hotel diagnostic blocked by "
                "dangerous S3 false positives."
            )
        )
