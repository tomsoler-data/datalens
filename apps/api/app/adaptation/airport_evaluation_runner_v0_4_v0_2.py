...
AIRPORT_EVALUATION_MANIFEST_RULE_VERSION = (
    "qlora_v0.4_airport_evaluation_manifest_v0.2"
)

AIRPORT_EVALUATION_MANIFEST_FREEZE_RULE_VERSION = (
    "qlora_v0.4_airport_evaluation_manifest_freeze_v0.2"
)

AIRPORT_EVALUATION_RECEIPT_RULE_VERSION = (
    "qlora_v0.4_airport_evaluation_receipt_v0.2"
)
...
COMPATIBILITY_DECISION_PATH = (
    ARTIFACT_DIR
    / "datalens_semantic_qlora_v0.4_"
      "airport_execution_compatibility_decision_v0.1.json"
)

MANIFEST_PATH = (
    ARTIFACT_DIR
    / "datalens_semantic_qlora_v0.4_"
      "airport_evaluation_v0.2_manifest.json"
)

MANIFEST_FREEZE_PATH = (
    ARTIFACT_DIR
    / "datalens_semantic_qlora_v0.4_"
      "airport_evaluation_v0.2_manifest_freeze.json"
)
...
TEST_REPO_PATH = (
    "apps/api/"
    "test_airport_evaluation_runner_v0_4_v0_2.py"
)
...
COMPATIBILITY_DECISION_REPO_PATH = (
    "apps/api/artifacts/adaptation/evaluation/"
    "datalens_semantic_qlora_v0.4_"
    "airport_execution_compatibility_decision_v0.1.json"
)
...
EXPECTED_COMPATIBILITY_DECISION_GIT_COMMIT = (
    "67f6225c00a8fb2c6f5b305231a7ec2e3fe06c5e"
)

EXPECTED_COMPATIBILITY_DECISION_SHA256 = (
    "0810a44dd55f41d10e739876b6aa1561"
    "3fc921446c2e1d1ff63971c4d1f980d1"
)
...
def _validate_prerequisite_evidence(
    ...
    _require_exact_sha(
        COMPATIBILITY_DECISION_PATH,
        EXPECTED_COMPATIBILITY_DECISION_SHA256,
        "Airport Compatibility Decision",
    )
...
def _validate_compatibility_decision(
    decision: Mapping[
        str,
        Any,
    ],
    ...
    if (
        ...
        !=
        "qlora_v0.4_airport_execution_compatibility_decision_v0.1"
    ):
        raise RuntimeError(
            "Airport compatibility decision rule mismatch."
        )
    ...
    if (
        ...
        !=
        "approved_before_airport_holdout_consumption"
    ):
        raise RuntimeError(
            "Airport compatibility decision is not approved."
        )
    ...
    if (
        ...
        !=
        "execution_compatibility_only"
    ):
        raise RuntimeError(
            "Airport compatibility decision scope changed."
        )
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "v0.1 execution history missing."
        )
    ...
    if (
        ...
        !=
        "superseded_before_holdout_consumption"
    ):
        raise RuntimeError(
            "v0.1 was not superseded before consumption."
        )

    for key in (
        "airport_cases_consumed",
        "airport_results_observed",
        "consumption_marker_created",
        ...
        if (
            ...
            raise RuntimeError(
                (
                    "v0.1 protected state changed: "
                    f"{key}"
                )
            )

    delta = decision.get(
        "approved_v0_2_change"
    )
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "Approved v0.2 compatibility delta missing."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Airport execution revision is not v0.2."
        )

    for key in (
        "semantic_protocol_changes",
        "holdout_changes",
        "prompt_changes",
        "relation_changes",
        "acceptance_gate_changes",
        "generation_budget_changes",
        "eos_changes",
        "model_changes",
        "adapter_changes",
        "training_changes",
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "Runtime compatibility change missing."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Compatibility function changed."
        )
    ...
    if (
        ...
        !=
        "set_return_dict_false_explicitly"
    ):
        raise RuntimeError(
            "Compatibility delta changed."
        )
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "Compatibility governance missing."
        )

    if (
        governance.get(
            "airport_holdout_consumption_budget_available"
        )
        is not True
    ):
        raise RuntimeError(
            "Airport consumption budget unavailable."
        )

    if (
        governance.get(
            "v0_1_must_not_be_executed"
        )
        is not True
    ):
        raise RuntimeError(
            "Historical v0.1 was re-enabled."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Greenhouse is no longer closed."
        )
...
def validate_static_contract(
    ...
    compatibility_decision = load_json_object(
        COMPATIBILITY_DECISION_PATH
    )

    _validate_compatibility_decision(
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "Airport compatibility binding missing."
        )

    if (
        compatibility_binding.get(
            "sha256"
        )
        !=
        EXPECTED_COMPATIBILITY_DECISION_SHA256
        ...
        raise RuntimeError(
            "Airport compatibility SHA binding changed."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Airport compatibility commit binding changed."
        )
...
def preflight_runtime(
    ...
    print(
        "=== DATALENS QLORA v0.4 AIRPORT RUNTIME PREFLIGHT v0.2 ==="
    )
    ...
    print(
        "DATALENS QLORA v0.4 AIRPORT RUNTIME PREFLIGHT v0.2: PASS"
    )
...
def execute_evaluation(
    ...
    print(
        "=== DATALENS QLORA v0.4 AIRPORT INDEPENDENT HOLDOUT v0.2 ==="
    )
    ...
    report = {
        "report_rule_version":
            "qlora_v0.4_airport_evaluation_report_v0.2",
    ...
    print()
    print(
        "DATALENS QLORA v0.4 AIRPORT INDEPENDENT HOLDOUT v0.2: COMPLETED"
    )
